...
「結局、どの Algorithm を使えばいいのか」に明確な回答はないが下記の観点が基準になることが多い。

1. Model の判断根拠の明確さが求められている
2. 精度の高さを求められるている
3. 学習時間の考慮

#### Model の判断根拠の明確さが求められてる

「Model がなぜその結果を出したか」を人間が追いかけること容易である必要があるので、その場合
...
「理由はわからなくて良いからできるだけ正確に予測したい」という Case の時。その場合、
...
といった Algorithm が選ばれることが多くなる。

#### 学習時間の考慮

実 Project となると Data 件数が数千万件、項目数が数百、数千になる Case がある。 その場合、学習にどれくらい時間がかかるかを考慮する必要がててくる。

- Logistic 回帰
- 決定木

は、仕組みが比較的単純なため学習時間という観点では有利になる。

##### ３つの観点かを整理した表

| Algorithm              | 実装方式  | 精度  | 説明性 | 学習時間 |
|------------------------|-------|-----|-----|------|
| Logistic 回帰            | 損失関数型 | △   | あり  | 速い   |
| Support Vector Machine | 損失関数  | 〇   | なし  | 遅い   |
| Neural Network         | 損失関数型 | ◎   | なし  | 遅い   |
| 決定木                    | 決定木型  | △   | あり  | 速い   |
| Random Forest          | 決定木型  | 〇   | なし  | 遅い   |
| XGBoost                | 決定木型  | ◎   | なし  | 遅い   |
